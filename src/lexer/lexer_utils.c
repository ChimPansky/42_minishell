...
/*   Updated: 2024/02/04 23:56:35 by tkasbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int read_word(t_msh *msh, t_lexer *lexer, t_string *str)
{
	char    quote_type;
...
int		read_tk_subshell(t_msh *msh,  t_lexer *lexer)
{
	...
	(void)msh;
	lexer->pos_in_input += 1;
	ft_putstr_fd("reading tk_subshell...", STDOUT_FILENO);

	// new_token = tokenlist_add_token(tokens_p, TK_WORD);
	// if (!new_token)
	// 	return (!SUCCESS);
	// if (read_word(msh, lexer->pos_in_input, &new_token->string) != SUCCESS)
	// 	return (!SUCCESS);
